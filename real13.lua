...
controller.Chatted:Connect(function(msg)
	...
	if string.sub(msg,1,1) == prefix then
		...
		if cmd == "bring" then
			...
			repeat
				char:WaitForChild("HumanoidRootPart").CFrame = CFrame.new(target.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame.Position) * CFrame.new(0,3,0)
				local args = {
					[1] = "Grabbing",
					[2] = false
				}
		...
		if cmd == "kill" then
			...
			wait(0.3)
			bot.Character:FindFirstChildOfClass("Humanoid"):UnequipTools()
			bot.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame = CFrame.new(99999,-workspace.FallenPartsDestroyHeight * 3,99999)
			bot.Character:FindFirstChildOfClass("Humanoid").RootPart.Anchored = true
			wait(1)
			repeat wait() 
				local args = {
					[1] = "Grabbing",
