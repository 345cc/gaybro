	...
	if string.sub(msg,1,1) == prefix then
		...
		if cmd == "kill" then
			...
			repeat
				...
				local args = {
					[1] = "Stomp",
					[2] = getNil("Stomp", "AnimationTrack"),
					[3] = game:GetService("Players").LocalPlayer.Character.BodyEffects
				}

				game:GetService("ReplicatedStorage"):WaitForChild("MainEvent"):FireServer(unpack(args))

				wait(.2)
			until tick() > Time + TimeToWait or target.Character:FindFirstChildOfClass("Humanoid").Health >= 15
			wait(0.2)
			bot.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame = oldpos
		end
		if cmd == "void" then
