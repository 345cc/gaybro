	...
	if string.sub(msg,1,1) == prefix then
				...
				repeat
					if RootPart and THumanoid then
						if BasePart.Velocity.Magnitude < 50 then
							...
							FPos(BasePart, CFrame.new(0, 1.5, 0) + THumanoid.MoveDirection * BasePart.Velocity.Magnitude / math.random(-5,5), CFrame.Angles(math.rad(Angle),0 ,0))
							task.wait()

							FPos(BasePart, CFrame.new(0, -1.5, 0) + THumanoid.MoveDirection * BasePart.Velocity.Magnitude / math.random(-5,5), CFrame.Angles(math.rad(Angle), 0, 0))
							task.wait()

							FPos(BasePart, CFrame.new(2.25, 1.5, -2.25) + THumanoid.MoveDirection * BasePart.Velocity.Magnitude / math.random(-5,5), CFrame.Angles(math.rad(Angle), 0, 0))
							task.wait()
		...
		if cmd == "bring" then
			...
			for i = 1,5 do
				...
				wait(1.4)
				for i = 1,30 do
					if target.Character:FindFirstChildOfClass("Humanoid").Health <= 10 then
						break
					end
			...
			if target.Character:FindFirstChildOfClass("Humanoid").Health >= 10 then print("failed to bring player (health error)") else end 
			repeat
		...
		if cmd == "kill" then
			...
			wait(0.3)
			bot.Character:FindFirstChildOfClass("Humanoid"):UnequipTools()
			bot.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame = CFrame.new(99999,0,99999)
			wait(1)
			repeat wait() 
				local args = {
					[1] = "Grabbing",
			...
			until 
			not target.Character:FindFirstChild("GRABBING_CONSTRAINT") or tick() > Time + TimeToWait
			bot.Character:FindFirstChildOfClass("Humanoid").RootPart.Anchored = false
			bot.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame = oldpos
		end
