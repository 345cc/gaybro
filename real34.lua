...
function fling(target, BasePart)
	...
	if TRootPart and THead then
		if (TRootPart.CFrame.p - THead.CFrame.p).Magnitude > 5 then
			...
			SFBasePart(target, TRootPart)
		end
	elseif TRootPart and not THead then
		SFBasePart(target, THead)
	else
		...
		if cmd == "aura" then
			Protect = game:GetService("RunService").Heartbeat:Connect(function()
				...
				for _, player in pairs(Players:GetPlayers()) do
					-- Ensure the player is not the LocalPlayer
					if player ~= bot then
						-- Calculate the distance between LocalPlayer and the current player
						local distance = (bot.Character.HumanoidRootPart.Position - player.Character.HumanoidRootPart.Position).Magnitude

						-- Check if the current player is closer than the nearest player found so far
				...
				fling(nearestPlayer, nearestPlayer.Character:FindFirstChildOfClass("Humanoid").RootPart)
			end)
		end
		if cmd == "unaura" then
			Protect:Disconnect()
