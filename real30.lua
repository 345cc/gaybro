...
controller.Chatted:Connect(function(msg)
	...
	if string.sub(msg,1,1) == prefix then
			...
			local SFBasePart = function(BasePart)
				...
				repeat
					if RootPart and THumanoid then
						angle = angle + 1
						FPos(BasePart, CFrame.new(0, 0, 0) + THumanoid.MoveDirection, CFrame.Angles(math.rad(angle),0 ,0))
						game:GetService("RunService").Heartbeat:Wait()
						FPos(BasePart, CFrame.new(0, 0, 0) + THumanoid.MoveDirection * BasePart.Velocity.Magnitude / 1, CFrame.Angles(math.rad(angle),0 ,0))
						game:GetService("RunService").Heartbeat:Wait()
						FPos(BasePart, CFrame.new(0, 0, 0) + THumanoid.MoveDirection * BasePart.Velocity.Magnitude / 1.25, CFrame.Angles(math.rad(angle),0 ,0))
						game:GetService("RunService").Heartbeat:Wait()
						FPos(BasePart, CFrame.new(0, 0, 0) + THumanoid.MoveDirection * BasePart.Velocity.Magnitude / 1.5, CFrame.Angles(math.rad(angle),0 ,0))
						game:GetService("RunService").Heartbeat:Wait()
						FPos(BasePart, CFrame.new(0, 0, 0) + THumanoid.MoveDirection * BasePart.Velocity.Magnitude / 1.75, CFrame.Angles(math.rad(angle),0 ,0))
						game:GetService("RunService").Heartbeat:Wait()
						FPos(BasePart, CFrame.new(0, 0, 0) + THumanoid.MoveDirection * BasePart.Velocity.Magnitude / 2, CFrame.Angles(math.rad(angle),0 ,0))
					else
						break
					end
		...
		if cmd == "bring" then
			...
			for i = 1,5 do
				...
				for i = 1,20 do
					...
					FPos(target.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame, CFrame.new(0, -2, 0) + target.Character:FindFirstChildOfClass("Humanoid").MoveDirection * target.Character:FindFirstChildOfClass("Humanoid").RootPart.Velocity.Magnitude / math.random(16,20))
					task.wait()
				end
				bot.Character:FindFirstChildOfClass("Humanoid"):UnequipTools()
				bot.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame = oldpos
		...
		if cmd == "hurt" then
			...
			for i = 1,5 do
				...
				for i = 1,20 do
					...
					FPos(target.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame, CFrame.new(0, RNG:NextNumber(-2,2), 0) + target.Character:FindFirstChildOfClass("Humanoid").MoveDirection * target.Character:FindFirstChildOfClass("Humanoid").RootPart.Velocity.Magnitude / math.random(16,20))
					task.wait()
				end
				bot.Character:FindFirstChildOfClass("Humanoid"):UnequipTools()
				bot.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame = oldpos
		...
		if cmd == "kill" then
			...
			until target.Character:FindFirstChild("GRABBING_CONSTRAINT") or tick() > Time + TimeToWait or target.Character:FindFirstChildOfClass("Humanoid").Health >= 15
			bot.Character:FindFirstChildOfClass("Humanoid"):UnequipTools()
			bot.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame = CFrame.new(9999,1,9999)
			wait(0.8)
			bot.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame = CFrame.new(9999,1,9999)
			local args = {
				[1] = "Grabbing",
		...
		if cmd == "protection" then
			Protect = game:GetService("RunService").Heartbeat:Connect(function()
				if bot.Character:FindFirstChildOfClass("Humanoid").Health >= 15 then
					repeat
						char:WaitForChild("HumanoidRootPart").CFrame = CFrame.new(controllerchar.Head.CFrame.Position) * CFrame.new(0,3,0)
						local args = {
							[1] = "Grabbing",
							[2] = false
						}
						game:GetService("ReplicatedStorage"):WaitForChild("MainEvent"):FireServer(unpack(args))
						wait(.1)
					until controllerchar.Character:FindFirstChild("GRABBING_CONSTRAINT") or controllerchar:FindFirstChildOfClass("Humanoid").Health >= 15
					char:WaitForChild("HumanoidRootPart").CFrame = controllerchar.HumanoidRootPart.CFrame * CFrame.new(0,50,0)
				end
			end)
		end
		if cmd == "unprotection" then
			Protect:Disconnect()
		end
